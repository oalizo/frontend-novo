...
-- Create improved function that preserves exact string values
...
BEGIN
  ...
  IF NEW.customer_track_id IS NOT NULL THEN
    -- Convert any number to string without scientific notation
    IF NEW.customer_track_id ~ E'^\\d+\\.?\\d*e[+-]?\\d+$' THEN
      -- Use TO_CHAR with FM999999999999999999999999 to avoid padding
      NEW.customer_track_id := TRIM(TO_CHAR(NEW.customer_track_id::numeric, 'FM999999999999999999999999'));
    END IF;
...
-- Fix existing values
UPDATE orders 
SET customer_track_id = TRIM(TO_CHAR(customer_track_id::numeric, 'FM999999999999999999999999'))
